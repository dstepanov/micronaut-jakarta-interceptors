...
What can be run are the *scenarios* those tests deploy. The `org.jboss.cdi.tck.interceptors.tests` package of
`jakarta.enterprise:cdi-tck-core-impl` carries the scenarios of the Interceptors specification - the beans, the
interceptor classes and the binding annotations its own authors wrote - and they are ordinary Jakarta Interceptors
code. The `cdi-tck-interceptors` module of this repository fetches those sources from Maven Central at build time,
compiles them with the annotation processor of this module and drives them from Micronaut, asserting what the
kit's own test classes assert. Nothing is vendored; the sources are Apache 2.0 licensed, as this project is.
...
rejects. A bean manager is stood in for where a scenario only takes one as an argument, and the scopes and producer
methods of Jakarta Contexts and Dependency Injection are read as their Micronaut equivalents, so that the scenarios
