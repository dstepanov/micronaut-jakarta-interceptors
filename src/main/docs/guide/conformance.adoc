...
visible in a test report as what is. What it cannot run declares a private interceptor method, which this module
rejects, or expects eager interceptor creation. For tests that expect an Arquillian deployment to fail, the bridge
reads the original deployment archive, compiles its classes with Micronaut's annotation processor and requires that
compilation to fail. The scopes and producer methods of Jakarta Contexts and Dependency Injection are read as their
Micronaut equivalents, so that the scenarios themselves can be created.
...
| Covered by a test of this repository | 124
| Differs from the specification, deliberately | 9
...
9 differences below, each of which is deliberate.
...
| 2.6 cb)
| A private interceptor method is rejected: it has no executable method to invoke

| 2.7 jb)
| A private interceptor method is rejected
