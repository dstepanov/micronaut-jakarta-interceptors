...
The kit runs complete: every test method of the package, none of them marked as unrunnable or excluded. For tests
that expect an Arquillian deployment to fail, the bridge reads the original deployment archive, compiles its classes
with Micronaut's annotation processor and requires that compilation to fail. The scopes and producer methods of Jakarta
Contexts and Dependency Injection are read as their Micronaut equivalents, so that the scenarios themselves can be
created.

Where the kit asks a bean manager which interceptors a set of bindings resolves to, the bridge answers: it walks the
interceptor classes and compares their bindings with this module's own binding comparison. There is nothing of the
module's to delegate that question to - it resolves an interception from the metadata of an element while the
application is compiled, and never compares a binding against an interceptor at runtime - so that count holds the
comparison to the specification rather than the module's resolution. The kit pairs each count with an invocation of
the bean it was taken over, and that half holds the module itself; see `InterceptorBindingResolutionTest`.
...
| Covered by a test of this repository | 128
| Covered by a test, with one exception described under Limitations | 2
| Differs from the specification, deliberately | 5
| Not applicable to a module without Jakarta Enterprise Beans, a deployment descriptor or a naming context | 27
...
5 differences below, each of which is deliberate, and the order a closing context destroys a `@Singleton` interceptor
in, which the two assertions about the destruction of interceptor instances name and <<limitations,Limitations>>
describes.
...
| 2.3 dd)
| An object is injected, and its post-construct callbacks run, before its around-construct interceptor methods
return rather than after

| 3.3 g)
| A binding declared on a final method is not reported; a final class, and a final method of a class carrying a
class level binding, are
...
| Covered, with one exception
| LifecycleInterceptionTest; an interceptor class the application declares a `@Singleton` is a bean of the context,
which a closing context may destroy before the bean whose destruction it interposes on, as `ShutdownOrderingTest`
holds and <<limitations,Limitations>> describes
...
| Covered
| InterceptorStateTest; TargetProxyStateTest for a bean proxied with a separate target, whose proxy and target
share one instance of each interceptor class from the post-construct event to the pre-destroy event
...
| Covered
| InterceptorLifeCycleTest, from the kit

| bb)
| These interceptor instances are destroyed if the target instance fails to be created or when it is removed.
| Covered
| InterceptorDestructionTest: the instances of an interceptor class without a scope of its own, and what they were
injected with, are destroyed with a singleton as the context closes, with an object passed to `destroyBean`, and
with an object whose constructor or post-construct callback throws; a `@Singleton` interceptor is left to the
context
...
| Covered, with one exception
| LifecycleInterceptionTest; the exception is an interceptor class the application declares a `@Singleton`, destroyed
with the other singletons of the context rather than with the object it intercepted: see `ShutdownOrderingTest` and
...
| Covered
| AroundConstructInstancesTest, AssociatedInstancesTest: every interceptor class the class of the object is bound to
is created and injected before the first around-construct method runs, including one that declares no
around-construct method and therefore takes no part in the construction. An interceptor class bound to one method of
the object alone is created as the object finishes being created; see the guide
...
| Differs
| Micronaut injects and initializes an object in the method that constructs it, which is what the last
around-construct method proceeds to: when `proceed()` returns to an around-construct method, the object has been
injected and its post-construct callbacks and post-construct interceptor methods have run.
InjectionInsideAroundConstructTest pins the order
|===
...
| Covered
| InvocationContextConformanceTest for around-invoke, AroundTimeoutTest for around-timeout
...
| Covered
| InvocationContextConformanceTest for a method, LifecycleContextDataTest for an event
...
| Covered
| InvocationContextTest, LifecycleInterceptionTest, ProducedMemberLookupTest
...
| Covered
| LifecycleProceedTest, LifecycleInterceptionTest, AroundConstructTest
...
| Covered
| InvocationContextConformanceTest, AroundConstructTest
...
| Covered
| InvocationContextConformanceTest; TypedBindingTest for a repeated binding, before and after the set is built, and from a construction and a lifecycle event
|===
...
| Covered
| ExceptionsTest; PrivateInterceptorMethodExceptionsTest for private around-invoke, around-timeout and post-construct
methods, which are reached reflectively
...
| Covered
| BindingReplacementTest; RepeatedProceedTest, where more interceptors follow, and
RepeatedProceedThroughMicronautAdviceTest, where a Micronaut advice follows
...
| Covered
| AccessLevelTest; InterceptorMethodsOfASuperclassInAnotherPackageTest, where the protected method is
declared by a superclass in another package
...
| Covered
| SelfConstructTest: a class declaring @AroundConstruct is read as an interceptor class, and the method does not interpose on the construction of the class itself
...
| Test package level access.
| Covered
| AccessLevelTest; InterceptorMethodsOfASuperclassInAnotherPackageTest, where the package private
method is declared by a superclass in another package
...
| Covered
| LifecycleExceptionTest; AroundConstructExceptionsTest for an around-construct method, which may throw what the
...
| Covered
| StructuralConformanceTest; TargetProxyStateTest for a bean proxied with a separate target, including one a
factory produced
|===
...
| Covered
| InterceptorBindingResolutionTest.testLifecycleInterceptorBindings, whose bean reaches one of its bindings only through a stereotype it inherits; CdiStereotypeRulesTest
|===
...
| Covered
| CdiBindingRulesTest; SharedSuperclassBusinessMethodTest, where two classes bound differently inherit one method,
and FactoryMethodsOfOneClassTest, where two factory methods bind one class differently; ProducerMethodsTest for a
static factory method and for a factory that interposes on its own business methods
...
| Covered
| InterceptorValidationTest: a final class carrying a class level binding, and a final method of such a class,
are both reported as the class is compiled; FinalElementsTest, for a final method of every access level the class
...
| Differs
| A binding declared on the method itself is not reported: Micronaut refuses a class it cannot proxy and a method
it cannot override, and a final method carrying a binding of its own is neither - the class is proxied and the
method is left out of the proxy, so the binding simply never applies
|===
...
| Covered
| AroundInvokeTest; ProtectedBusinessMethodTest, where the business method is `protected`
...
| Covered
| BindingMembersTest; RepeatedBindingTest where the binding type is repeatable, each occurrence of which is matched
as a binding of its own, as the reference implementation matches one
...
| Differs
| An array-valued member is supported and compared by its contents: ArrayBindingMemberTest for an array of objects, PrimitiveArrayBindingTest for each primitive array type
...
| Covered
| InterceptorValidationTest, including a conflict reached through binding annotations declared on each other in
either order; InvalidStereotypeInterceptorBindingAnnotationsTest and InvalidTransitiveInterceptorBindingAnnotationsTest
of the kit. A member excluded from the binding is not one the instances disagree about, so two of them differing only
in such a member are accepted, which the same test asserts. A repeatable binding type is not two instances of one
type either: each occurrence of it is a binding of its own, as RepeatedBindingTest asserts and as the reference
implementation resolves one
|===
...
| Covered
| HierarchyOrderTest; PrivateInterceptorMethodsOfOneNameTest, where a class and its superclass each declare a private
interceptor method of one name and signature
