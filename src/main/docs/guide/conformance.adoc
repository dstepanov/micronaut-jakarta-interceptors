...
visible in a test report as what is. What it cannot run needs a `jakarta.enterprise.inject.spi.BeanManager` to
resolve or count interceptors, or declares a private interceptor method, which this module rejects. A bean manager
is stood in for where a scenario only takes one as an argument, and the scopes and producer methods of Jakarta
