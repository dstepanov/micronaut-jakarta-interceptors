...
rejects, expects eager interceptor creation, or expects an Arquillian deployment to fail. The last case is tested
by compiling the deployment and requiring compilation to fail. The scopes and producer methods of Jakarta Contexts
and Dependency Injection are read as their Micronaut equivalents, so that the scenarios themselves can be created.
